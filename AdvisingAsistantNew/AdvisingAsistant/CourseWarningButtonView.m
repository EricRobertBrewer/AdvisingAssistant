...
- (void)didTapWarning:(id) selector {
    if ([prereqs count] != 0)
    {
        ...
        [AANotify present:CKNotifyAlertTypeInfo title:@"Prerequisites not met" body:[NSString stringWithFormat:@"Courses required: %@",courses] duration:5];
    }
    ...
    if ([coreqs count] != 0)
    {
        ...
        [AANotify present:CKNotifyAlertTypeInfo title:@"Corequisites not met" body:[NSString stringWithFormat:@"Courses required: %@",courses] duration:5];
    }
}
